...
export default {
  ...
  methods: {
    ...
    async handleEdit(index, row) {
      this.$router.push({ path: '/admin_main/course/create', query: { id: row._id } })
    },
    //删除
    handleDelete(row) {
      this.$confirm(`是否删除此条数据?`, '危险', {
        ...
        let success = await this.$http.delete(`rest/course/${row._id}`);
    ...
    async paging(page) {
      const res = await this.$http.get(`rest/course/${page}`);
