<template>
	<div class="school_exam">
		<div class="border">
			<van-nav-bar title="课程考试" left-text="返回" left-arrow @click-left="onClickLeft">
			</van-nav-bar>
		</div>
		...
		<h2 style="text-align: center; color: #333" v-else>
			暂时没有试卷
		</h2>
	</div>
</template>
		...
		methods: {
			...
			enterClick(index) {
				Dialog.confirm({
					...
					.then(() => {
						...
						// this.$router.replace({ name: 'test_parper', query: { data: JSON.stringify(this.tableData[index]) } });
			...
			async fetch() {
				...
				if (localStorage.s_user) {
					...
					const res = await this.$http.get(`rest/test_paper/save/${sId}/${this.id}`);
