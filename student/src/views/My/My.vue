<template>
	<div>
		<div class="border">
			<van-nav-bar title="个人中心" right-text="修改密码" @click-right="editPassword"> </van-nav-bar>
		</div>
		<van-form @submit="onSubmit" class="form" scroll-to-error colon>
			<!-- 学号 -->
			<div class="borderItem">
				<van-cell-group>
					<van-field v-model="model.sId" label="学号" placeholder="学号" readonly />
				</van-cell-group>
			</div>
			<!-- 姓名 -->
			<div class="borderItem">
				<van-cell-group>
					<van-field v-model="model.name" label="姓名" placeholder="输入姓名(必填)"
						:rules="[{ required: true, message: '请填写姓名' }]" />
				</van-cell-group>
			</div>
			<!-- 性别 -->
			<div class="borderItem">
				<van-field name="uploader" label="性别" placeholder="输入性别(必填)"
			...
			<!-- 学院 -->
			<div class="borderItem">
				<van-field readonly clickable name="picker" :value="collegeName" label="学院" placeholder="选择学院"
					:rules="[{ required: true, message: '请选择学院' }]" @click="showPickerCollege = true" />
				<van-popup v-model="showPickerCollege" position="bottom">
					<van-picker title="学院" show-toolbar :columns="colleges" value-key="name" @confirm="onConfirmCollege"
						@cancel="showPickerCollege = false" />
				</van-popup>
			</div>
			<!-- 专业 -->
			<div class="borderItem">
				<van-field readonly clickable name="picker" :value="professionName" label="专业" placeholder="选择专业"
					:rules="[{ required: true, message: '请选择专业' }]" @click="showPickerProfession = true" />
				<van-popup v-model="showPickerProfession" position="bottom">
					<van-picker title="专业" show-toolbar :columns="professions" value-key="name"
						@confirm="onConfirmProfession" @cancel="showPickerProfession = false" />
				</van-popup>
			</div>
			<!-- 班级 -->
			<div class="borderItem">
				<van-field readonly clickable name="picker" :value="className" label="班级" placeholder="选择班级"
					:rules="[{ required: true, message: '请选择班级' }]" @click="showPickerClass = true" />
				<van-popup v-model="showPickerClass" position="bottom">
					<van-picker title="班级" show-toolbar :columns="classes" value-key="name" @confirm="onConfirmClass"
						@cancel="showPickerClass = false" />
				</van-popup>
			</div>
			<!-- 出生日期 -->
			<div class="borderItem">
				<van-field readonly clickable name="datetimePicker" :value="dateOfBirth" label="出生日期"
					placeholder="选择出生日期" :rules="[{ required: true, message: '请选择出生日期' }]"
					@click="showPickerDateOfBirth = true" />
				<van-popup v-model="showPickerDateOfBirth" position="bottom">
					<van-datetime-picker type="date" @confirm="onConfirmDateOfBirth" :min-date="minDate"
						:max-date="maxDate" @cancel="showPickerDateOfBirth = false" />
				</van-popup>
			</div>
			<!-- 年龄 -->
			<div class="borderItem">
				<van-cell-group>
					<van-field v-model="model.age" label="年龄" placeholder="年龄" readonly />
				</van-cell-group>
			</div>
			<div style="display: flex;width: 100%;">
				<div style="margin: 16px;width: 160px;">
					<van-button class='but' round block type="info" native-type="submit">保存</van-button>
				</div>
				<div style="margin: 16px;width: 160px;">
					<van-button class='but1' round block  @click="exitLogin" native-type="button">退出</van-button>
				</div>
			</div>
		</van-form>
	</div>
	...
	export default {
		...
		data() {
			return {
				...
				classes: [], //请求班级获取到的数据
				minDate: new Date(1999, 0, 1), //时间的范围
				maxDate: new Date(),
			};
		},
		methods: {
...
<style scoped>
	...
	::v-deep .van-field__body:nth-child(3){
		border:0px;
	}
