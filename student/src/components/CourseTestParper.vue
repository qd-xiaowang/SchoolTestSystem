...
export default {
  ...
  methods: {
    ...
    async autoSubmit() {
      ...
      data.course_test_paper = this.id;//当前的试卷
      data.profession_course = this.test_parper.profession_course;
      let list = await this.$http.post('rest/course_test_paper_score', data);
    ...
    async fetch() {
      const res = await this.$http.get('rest/course_test_paper/' + this.id);
